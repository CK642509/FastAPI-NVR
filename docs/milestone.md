...
> 階段編號與 `proposal.md`、`roadmap.md` 一致（5 階段）。

### Phase 1：Core (後端基礎錄影管線)

建立可運作的影像擷取與持續錄影後端，作為後續所有功能的基礎。無前端介面。

功能：
- 支援 webcam 與 RTSP 影像來源（透過 `.env` 設定）
- 持續錄影，分段儲存為 mp4（segment 長度可設定）

### Phase 2：POC (MJPEG 即時串流 + Vue3 前端 + 容器化)
...
- Vue3 前端 (即時畫面顯示 + 錄影開關)
- PostgreSQL schema 建立 (為 Phase 3 事件功能預備)
- Docker compose (Caddy + FastAPI + DB)

### Phase 3 (基礎影像分析 + 事件系統 + 通知系統)
